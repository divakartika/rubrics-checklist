...
from helper import dv_functions as func
...
def app(case, caps):
    ...
    for topic in topic_dict.index:
        st.subheader(topic)
        cat_length = len(topic_dict.loc[topic, 'Category'])
        for num in range(cat_length):
            st.write(f"**{topic_dict.loc[topic, 'Category'][num]}**") # bold text
            st.write(topic_dict.loc[topic, 'Explanation'][num])
            try:
                plot = topic_dict.loc[topic, 'Plot'][num].split(',')
                caption = topic_dict.loc[topic, 'Caption'][num]
                st.image(plot, caption=caption.split(',') if isinstance(caption, str) else caption)
            except:
                pass
